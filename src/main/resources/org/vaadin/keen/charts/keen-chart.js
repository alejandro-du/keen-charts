window.org_vaadin_keen_charts_KeenChart = function() {
    ...
    connector.draw = function(projectId, readKey, queryType, args, customArgs) {
        var client = new Keen({projectId: projectId, readKey: readKey});

        Keen.ready(function() {
            var query = new Keen.Query(queryType, JSON.parse(args));
            client.draw(query, element, JSON.parse(customArgs));
        });
    }
}
